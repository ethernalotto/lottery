...
let nonce = process.env.ETHERNALOTTO_NONCE_OVERRIDE ?
    parseInt(process.env.ETHERNALOTTO_NONCE_OVERRIDE, 10) :
    null;
...
async function retry(action) {
  let attempts = parseInt(process.env.ETHERNALOTTO_DEPLOYMENT_ATTEMPTS, 10);
  while (attempts-- !== 0) {
    try {
      return await action();
    } catch (e) {
  ...
  throw new Error(`action failed after ${process.env.ETHERNALOTTO_DEPLOYMENT_ATTEMPTS} attempts`);
}
...
exports.deploy = async (name, args = [], libraries = {}) => {
  return await retry(async () => {
    ...
    await transaction.wait(process.env.ETHERNALOTTO_CONFIRMATIONS);
